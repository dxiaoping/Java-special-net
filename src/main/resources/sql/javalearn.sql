...
VALUES (15574902295, 0, '小平', '123456');

insert into user (phone, role, name, password)
VALUES (15512345678, 1, '教师1', '123456');
...
create table `menu` (
  ...
  `parent_id` bigint(20) not null comment '父目录Id',
  `name` varchar(20) not null comment '目录名称',
  `rank` int comment '目录链接',
  primary key (id)
) comment = '目录表';

# drop table example;
create table `example`(
  `id` bigint(20) not null comment '实例Id',
  `user_id` bigint(20) not null comment '用户手机号',
  `menu_id` bigint(20) not null comment '依附的目录Id',
  `title` varchar(20) not null comment '实例题目',
  `content` varchar(3000) not null comment '实例内容',
  `interpret` varchar(3000) not null comment '实例讲解',
  `run_result` varchar(3000) not null comment '运行结果',
  `attachment` varchar(300) not null comment '代码附件',
  `scan_count` int not null default 0 comment '浏览次数',
...
create table `resources`(
  ...
  `user_id` bigint(20) not null comment '用户手机号',
  `menu_id` bigint(20) not null comment '依附目录Id',
  `description` varchar(1000) comment '资源描述',
  `url` varchar(500) not null comment '资源路径',
  `res_type` int not null comment '资源类型',
  `download_count` int not null default 0 comment '资源下载次数',
  `create_time` datetime not null comment '资源上传时间'
) comment = '资源表';

alter table resources change column `url` `url` varchar(500) not null comment '资源路径';
drop table known;
create table `known`(
  `id` bigint(20) not null comment '知识点id',
  `menu_id` bigint(20) not null comment '依附目录Id',
  `user_id` bigint(20) not null comment '用户手机号',
  `name` varchar(20) not null comment '知识名',
  `text` varchar(1000) comment '知识文本',
  `attachment` varchar(200) comment '附件路径',
  `scan_count` int not null comment '浏览次数',
  `download_count` int not null default 0 comment '下载次数',
  `create_time` datetime not null comment '创建时间'
) comment = '知识表';
